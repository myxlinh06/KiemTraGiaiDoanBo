...
def safe_ceil(value, default=0):
    """Làm tròn lên (ceil) giá trị float, nếu lỗi trả về default"""
    try:
        if value is None:
            return default
...
def classify_cow(doc):
    ...
    pregnant_days = safe_ceil(doc.get("SoNgayMangThai", 0))
    ...
    # ===== RULE =====
    if age_days is not None and group not in ["LoaiThai", "XuatBan"]:
        ...
        if 540 < age_days <= 600 and gender == "đực" and group == "BoVoBeo":
            return "BoNuoiThitBCT18_20"
        if 540 < age_days <= 600 and gender == "cái" and pregnant_days == 0 and group == "Bo":
            return "BoNuoiThitBCT18_20"
        ...
        if 600 < age_days <= 690:
            if gender == "đực" and group != "BoDucGiong":
                return "BoVoBeoNho"
            if gender == "cái" and pregnant_days == 0 and group != "Bo":
                return "BoVoBeoNho"
    ...
    if gender == "cái" and group not in ["LoaiThai", "XuatBan"]:
        ...
        if (
            (nghiep_vu == "PhoiGiong" and cong_viec == "Rút vòng + Tiêm PGF2α 2ml")
            or cong_viec == "Đặt vòng + Tiêm GnRH 2.5ml"
            or cong_viec == "Tiêm GnRH 2.5ml"
        ) and group != "BoVoBeo":
...
#         print("Ngày sinh bê:", ngay_sinh_be)
